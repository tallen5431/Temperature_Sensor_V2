...
- **Firmware target:** ESP32-C3 (SuperMini), firmware **v2.8.1**, protocol v1 (FQBN `esp32:esp32:esp32c3`).
...
- **Future / not in current firmware:** MAX31855 K-type thermocouple (SPI) and
  ...
  They are **not** implemented in v2.8.1 and are not populated on shipping units.
...
docs described are **not implemented in firmware v2.8.1** — there is no build
