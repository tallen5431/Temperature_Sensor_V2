...
```
Setpoint (ESP32) ──mDNS advertise──▶ ProbeDiscovery (zeroconf browse)
                                                 │
                                                 ▼
                                        AutoProvisioner (every 10s)
                                                 │  POST /provision {server_url, token, interval_ms}
                                                 ▼
Setpoint ──POST /api/ingest {temperature_c, probe_id, timestamp}──▶ Hub API
...
| `firmware/` | Firmware contract (`src/protocol.h`) + factory-flash / QC tooling (`factory_flash.py`). The ESP32 Setpoint firmware itself is the Arduino sketch `esp32_temp_probe/esp32_temp_probe.ino` (deep-sleep/battery), built with the Arduino toolchain / arduino-cli. |
...
`setpoint_probe_temperature_celsius`, plus health counters). Enabled by default; disable with
...
probe) a retained HA discovery config to `<discovery_prefix>/sensor/setpoint_<probe_id>/config`,
