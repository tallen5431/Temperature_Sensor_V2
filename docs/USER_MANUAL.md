...
When a reading crosses your limits, Setpoint can notify you. To receive emails or a webhook alert, an installer or maker needs to fill in the notification settings (email server or webhook address) once; see the developer documentation. Alerts are spaced out (debounced) so you aren't flooded with repeats.
