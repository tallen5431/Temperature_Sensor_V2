...
- A **ThermaProbe** sensor with a temperature probe on a lead.
- A **USB power adapter** and cable.
- A **label / QR sticker** on the unit. It shows the probe's name (like **ThermaProbe-9A3F2C**), a **setup Wi-Fi password**, and a **setup secret**. Keep this label — you'll use it during setup.

---

## 2. Start the hub on your PC

1. On the ThermaHub PC:
...
1. Plug the ThermaProbe into the USB power adapter and into a wall outlet (or a USB battery pack).
...
## 4. Join the probe's setup Wi-Fi (ThermaProbe SoftAP)

1. On your phone or laptop, open the **Wi-Fi settings**.
2. Look for a network named **`ThermaProbe-XXXXXX`** (the same code as on the unit's label).
...
4. The probe saves the network and restarts. Your phone will drop the temporary `ThermaProbe-XXXXXX` network — that's a good sign. **Reconnect your phone to your normal Wi-Fi.**
...
Default names like `ThermaProbe-9A3F2C` are hard to remember. Give each probe a friendly name:
...
When a reading crosses your limits, ThermaHub can notify you. To receive emails or a webhook alert, an installer or maker needs to fill in the notification settings (email server or webhook address) once; see the developer documentation. Alerts are spaced out (debounced) so you aren't flooded with repeats.
...
**I can't find the `ThermaProbe-XXXXXX` setup network.**
...
Confirm your phone is connected to the `ThermaProbe-XXXXXX` network (not your home Wi-Fi), then reload the page.

**The probe shows a strange value like 85 °C or −127 °C.**
Those are sensor fault readings, usually from a loose or disconnected probe tip. ThermaHub ignores them automatically. Check that the probe lead is firmly seated; if it persists, the sensor may need attention.
