; Inno Setup script for the TempSensor hub — wraps the PyInstaller onedir build
; (dist\temperature-hub\) into a one-click Windows installer.
;
; Build (after packaging\build.bat has produced dist\temperature-hub\):
;   iscc /DAppVersion=2.4.0 packaging\windows\tempsensor.iss
; Output: dist\installer\TempSensor-Setup-<version>.exe
;
; Design notes:
;  * PrivilegesRequired=lowest — installs per-user, no admin prompt. Combined
;    with the app writing its data to %LOCALAPPDATA%\TempSensor (see app.py
...
#define AppPublisher "TempSensor"
...
OutputBaseFilename=TempSensor-Setup-{#AppVersion}
Compression=lzma2
