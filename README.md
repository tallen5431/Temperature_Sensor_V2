...
![Hub](https://img.shields.io/badge/hub-v2.7.2-brightgreen)
![Firmware](https://img.shields.io/badge/firmware-v2.9.4-brightgreen)
...
A probe has **two** cadences. `probe_intervals` (per probe, on **Devices → Edit**) is how often it *transmits*. `probe_sample_sec` (the hub-wide default, on **Settings → Probes**) is how often it *reads the sensor in between*, and `probe_samples` overrides that per probe, also on **Devices → Edit** — a walk-in on mains can afford to look every 10 s where a battery probe in a remote freezer should not, and one number for the fleet forces the slowest of those onto everything. Blank inherits the default.

The between-report check applies only to probes that have a min or max limit **and** report less often than they check. The probe then keeps its radio off between reports, buffers what it reads, and transmits immediately if a reading crosses a limit. So a probe reporting every 15 minutes still catches a thawing freezer within one check, and the buffered readings arrive with the next report at no extra battery cost. The Edit dialog states which of the two a given probe is on, and the Settings field names the probes it currently reaches — both conditions are easy to miss, and the shipped defaults (60 s check, 5 s reporting interval) do not meet them.
