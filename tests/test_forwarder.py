...
# --- The receiver's BYTE limit, which a row limit alone cannot honour --------

def test_default_batch_of_humidity_readings_would_be_rejected_whole(store, monkeypatch):
    """The shipped default is not safe on row count alone. 500 readings encode
    to ~62 KB bare, but ~92 KB once a grow probe adds humidity and battery —
    past the receiver's 64 KB limit, so every request 413s and forwarding stops
    dead. The batch is trimmed to what will actually be accepted instead."""
    from core.forwarder import MAX_FORWARD_BODY_BYTES, batch_body

    db, cfg = store
    now = datetime.datetime.now()
    for i in range(500):
        db.append((now - datetime.timedelta(seconds=500 - i)).isoformat(
            timespec="milliseconds"), 4.0, 39.2, "Setpoint-9A3F2C",
            humidity=55.25, battery=87.5)
    bodies = []

    def accept(url, token, site, rows, events=(), timeout=20.0):
        bodies.append(batch_body(rows, events))
        return 200

    monkeypatch.setattr("core.forwarder.post_batch", accept)

    sent = UpstreamForwarder(db, cfg).run_once()

    assert 0 < sent < 500, "the whole batch must not be sent as one over-limit body"
    assert len(bodies[0]) <= MAX_FORWARD_BODY_BYTES
    # The cursor advanced only through what was actually accepted.
    assert int(db.meta_get("forwarder.last_id", "0")) == sent


def test_a_trimmed_batch_still_drains_fast(store, monkeypatch):
    """Trimming for size leaves more waiting just as surely as filling the row
    limit does. Judging "full" on the trimmed count would sleep a whole interval
    with the rows it had just dropped still queued."""
    db, cfg = store
    cfg.set("upstream", {**cfg.get("upstream"), "batch": 1000, "interval_sec": 30})
    now = datetime.datetime.now()
    for i in range(700):
        db.append((now - datetime.timedelta(seconds=700 - i)).isoformat(
            timespec="milliseconds"), 4.0, 39.2, "Setpoint-9A3F2C",
            humidity=55.25, battery=87.5)
    monkeypatch.setattr("core.forwarder.post_batch", _Capture(200))
    stop = _RecordingStop(cycles=1)

    UpstreamForwarder(db, cfg, stop).run()

    assert stop.delays == [1], "a size-trimmed batch must not sleep the interval"


def test_trimming_only_ever_drops_from_the_tail(store, monkeypatch):
    """Cursors advance to the last id accepted, so dropping anything but a
    suffix would step the cursor past records that were never sent."""
    db, cfg = store
    _seed(db, 400, probe="Setpoint-9A3F2C")
    for _ in range(400):
        db.record_event("high", "Setpoint-9A3F2C", 12.0, 8.0)
    cap = _Capture(200)
    monkeypatch.setattr("core.forwarder.post_batch", cap)
    ...
    call = cap.calls[0]
    assert call["ids"] == list(range(1, len(call["ids"]) + 1))
    assert call["event_ids"] == list(range(1, len(call["event_ids"]) + 1))
    assert int(db.meta_get("forwarder.last_id", "0")) == call["ids"][-1]
    assert int(db.meta_get("forwarder.last_event_id", "0")) == call["event_ids"][-1]


def test_fit_batch_keeps_events_when_no_reading_fits_beside_them():
    """Events are the record an auditor asks for, so readings give way first —
    and events are only trimmed when not one reading fits alongside them."""
    from core.forwarder import batch_body, fit_batch

    rows = [(i, "2026-01-01T00:00:00.000", 0, 4.0, 39.2, "P" * 32, None, None, None)
            for i in range(1, 40)]
    events = [(i, "2026-01-01T00:00:00", 0, "high", "P", 12.0, 8.0) for i in range(1, 6)]
    budget = len(batch_body([], events)) + 10   # room for the events, not a row

    kept_rows, kept_events = fit_batch(rows, events, max_bytes=budget)
    assert kept_rows == []
    assert kept_events == events


def test_fit_batch_is_logarithmic_not_quadratic():
    """The obvious pop-one-and-re-encode loop measured 480 encodes and ~0.5 s of
    CPU per cycle at a 1000-row batch — on the forwarder thread, every interval,
    forever. Binary search must stay in the low tens."""
    from core.forwarder import batch_body, fit_batch
    import core.forwarder as fwd

    rows = [(i, "2026-08-04T10:15:32.500", 1785837932.5, -18.375, -1.075,
             "Setpoint-9A3F2C", None, None, None) for i in range(1, 1001)]
    encodes = [0]
    real = fwd.batch_body
    ...
    fwd.batch_body = counting
    try:
        kept, _ = fit_batch(rows, [])
    finally:
        fwd.batch_body = real
    assert 0 < len(kept) < 1000
    assert len(batch_body(kept, [])) <= fwd.MAX_FORWARD_BODY_BYTES
    assert encodes[0] <= 20, f"{encodes[0]} encodes — the fit is not a binary search"
