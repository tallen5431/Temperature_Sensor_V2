...
def test_latest_readings_record_and_snapshot():
    lr = LatestReadings()
    lr.record("TempSensor-1", 21.5, ts_epoch=1000.0)
    snap = lr.snapshot()
    assert snap["TempSensor-1"]["temp_c"] == 21.5
...
def test_render_prometheus_format():
    ...
    latest = {"TempSensor-9A3F2C": {"temp_c": 4.25, "ts": 1000.0}}
    out = render_prometheus(health, latest, probes_count=2, version="2.0.0")
    assert "# TYPE tempsensor_probe_temperature_celsius gauge" in out
    assert 'tempsensor_probe_temperature_celsius{probe_id="TempSensor-9A3F2C"} 4.250' in out
    assert "tempsensor_rows_written_total 3" in out
    assert "tempsensor_healthy 1" in out
    assert 'tempsensor_up{version="2.0.0"} 1' in out
...
def test_render_prometheus_includes_humidity_and_vpd():
    ...
    assert 'tempsensor_probe_humidity_percent{probe_id="P1"} 55.00' in out
    assert 'tempsensor_probe_vpd_kpa{probe_id="P1"} 1.400' in out
    # Temperature-only probes must NOT emit humidity/vpd lines.
    out2 = render_prometheus(health, {"P2": {"temp_c": 4.0, "ts": 1000.0}}, 1, "2.0.0")
    assert "tempsensor_probe_humidity_percent" not in out2


def test_ingest_records_latest_reading(tmp_db):
    client, _ = make_client(tmp_db, token="")
    client.post("/api/ingest", json={"temperature_c": 7.7, "probe_id": "TempSensor-METRIC"})
    assert LATEST.snapshot().get("TempSensor-METRIC", {}).get("temp_c") == 7.7


def test_ingest_records_humidity_and_vpd(tmp_db):
    client, _ = make_client(tmp_db, token="")
    client.post("/api/ingest", json={"temperature_c": 25, "humidity_pct": 50, "probe_id": "TempSensor-HUM"})
    entry = LATEST.snapshot().get("TempSensor-HUM", {})
